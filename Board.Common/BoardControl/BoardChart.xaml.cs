...
namespace Board.Controls.BoardControl
{
    ...
    public partial class BoardChart : UserControl, IDesigner, IWorker
    {
        ...
        public void StartWork()
        {
            ...
            foreach (DesignerChartDataSerie dcds in ChartDataSeries) 
            {
                if(string.IsNullOrEmpty(dcds.DataSourceKey))
                    continue;
                DispatcherTimer timer = new DispatcherTimer();
                timer.Interval = new TimeSpan(0, 0, 0, 0, dcds.DataAccessTimeSpan);
                timer.Tick += (s, e) =>
                {
                    try
                    {
                        ...
                        foreach (DataSeries ds in uChart.Series)
                        {
                            if (ds.Uid == dcds.Name)
                            {
                                ds.DataPoints = new DataPointCollection(dpList);
                                break;
                            }
                        }

                        if (DataSourceManager.GetDataSource(dcds.DataSourceKey).TimeSpan <= 0)
                            timer.Stop();
                    }
                    catch (Exception ex) 
                    {
                        LogManager.WriteLog("执行图标定时器任务出错," + ex.Message);
                    }
